...
exports.Porter = function Porter(resources, generateDOC) {
    ...
    this.describe = function (cb) {
      _describe(resources, undefined,cb);
    }

    function _describe(resources, path, cb) {
      for (var resource in resources) {
          if (resources.hasOwnProperty(resource)) {
            if (Object.prototype.toString.call(resources[resource]) == "[object Array]") {
              cb(gen_doc( resources[resource],path+"."+resource ));
            }else{
              _describe(resources[resource], path+"."+resource, cb)
            }
          }
      }
    }
    ...
    function gen_doc (data, path) {
      ...
      var url= data[1].match(/:([a-z_-]*)/gi);
      var urls={};
      if (url) {
        for (var i = url.length - 1; i >= 0; i--) {
          Object.defineProperty( urls, url[i].replace(":",""), {value : 10,  
                               writable : true,  
                               enumerable : true,  
                               configurable : true} );
        };
      ...
      doc = doc+"### Resource: " +data[0].toUpperCase()+" "+ self.options.protocol+"://"+self.options.host+self.options.endpoint +data[1] + "\n\n";

      if (doc_arguments) {
        doc = doc+"### Required payload:\n"+DumpObjectIndented(doc_arguments)+"\n\n";
      }
      if (doc_help) {
        doc = doc+"### Description:\n"+doc_help+"\n\n";
      }

      doc = doc+"### Example: \n    "+path.replace("undefined","[instance]")+"(\n      "+String(JSON.stringify(urls)).replace("undefined","null")+",\n      "+String(JSON.stringify(doc_arguments)).replace("undefined","{}")+",\n      function(error, response) {\n        // do something...\n      }\n    );"
